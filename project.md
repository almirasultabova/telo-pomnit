...
**Анастасия Хорошавцева** — психосоматика, телесная терапия, работа с бессознательным.
...
Запуск нового потока намечен на октябрь 2026. Точные даты и расписание встреч — пока не определены. На лендинге сейчас показан плейсхолдер (старт 15 октября, ранняя цена до 11 октября) — обновить на реальные даты, когда определятся (`landing_final.html`, три места с `new Date('2026-10-...')`).

Когда даты будут известны, нужно:
- создать новый `Stream` в БД (по образцу `backend/scripts/create-stream.js`)
- обновить `tg-app/js/data.js` (`DATA.program.schedule`) на актуальные встречи
- обновить даты/цены на лендинге (`landing_final.html`)
- при наличии Zoom-ссылки — заполнить `zoomUrl` (сейчас `null`)

### Запись на поток — сейчас через лист ожидания, не оплату

С 2026-08-24 кнопки записи на лендинге ведут не на оплату, а в лист ожидания: модалка (email + Telegram) → `POST /waitlist` → запись в БД (`WaitlistEntry`) → уведомление админам в Telegram. Оплата через ЮКассу (`payment.js`, `POST /create-payment`) технически работает, но никуда на сайте не подключена — решили пока не собирать оплату, а собрать контакты и написать вручную.
...
| `src/routes/payment.js` | Создание платежа ЮКасса + webhook (IP-фильтр) + авто-зачисление. Сейчас не используется на лендинге — вместо оплаты лист ожидания (`waitlist.js`), см. «Запись на поток» ниже |
| `src/routes/waitlist.js` | Заявка в лист ожидания (email + Telegram, без оплаты) → уведомление админам в боте |
